...
Take a video of yourself, and watch it back to check your form. Here's a [video](https://youtu.be/hzLvXIvhp9A?t=366) from Nate KG explaining the correct form.
...
This repository now contains a minimal [Astro](https://astro.build) site that renders this guide as a GitHub Pages website.

Quickstart

1. Update `astro.config.mjs` and replace `<username>` with your GitHub username so `site` becomes `https://<username>.github.io`.
2. Commit and push to the `master` branch. The GitHub Action (`.github/workflows/deploy.yml`) will build and deploy the site to GitHub Pages.
...
- If you want the site at `https://<username>.github.io/` (user site), set `base` to `/` and name the repository `<username>.github.io`.
- To publish at a custom domain, add a `public/CNAME` file and update `site` in `astro.config.mjs` to use your custom domain.
# Jump Rope Guide
A simple guide for learning jump rope (and freestyle).
- [Basics and FAQ](#basics-and-faq)
- [Buying Guide](#buying-guide)
...
Weighed ropes are used when you want to get an arm and upper-body workout. They are not ideal for tricks, but you can still do simple tricks like side swings, crosses, and double unders.
- [Crossrope.com](https://www.crossrope.com/pages/lp-classic/)
- [BuyJumpRopes.net](https://elitejumps.co/collections/heavy-jump-ropes)
#### Speed
Here's an example of what [speed](https://www.youtube.com/watch?v=0ergYjOG5Vk&ab_channel=DylanPlummer) can look like.
- [Buyjumpropes](https://elitejumps.co/collections/double-under-jump-ropes)
